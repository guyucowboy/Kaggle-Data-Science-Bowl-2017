...
def convolutional_neural_network(x):
    ...
    weights = {'W_conv1':tf.Variable(tf.random_normal([3,3,3,1,32])),
               ...
               'W_fc':tf.Variable(tf.random_normal([373248,1024])),
...
def train_neural_network(x):
    num_of_cancer_count = 0  # Added by Yaqin Huang 4/22/2017
    num_of_normal_count = 0  # Added by Yaqin Huang 4/22/2017
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            for data in sorted_train_data:
                ...
                try:
                    ...
                    if num_of_cancer_count / (
                                num_of_cancer_count + num_of_normal_count) >= 0.75:  # Added by Yaqin Huang 4/22/2017
        ...
        # Write to CSV
        f = open('stage1_submission'+ str(IMG_SIZE_PX) + '_' + str(SLICE_COUNT) + '.csv', 'wt')
        writer = csv.writer(f)
...
def score_cal():
	...
	result = pd.read_table('stage1_submission'+ str(IMG_SIZE_PX) + '_' + str(SLICE_COUNT) + '.csv', sep=',', index_col = 0)
